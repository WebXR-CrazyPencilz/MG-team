...
// ── LIVE DATA GLOBALS ───────────────────────────────
// CLIENTS/PROJECTS are read directly by form.js and forceentry.js,
// and reassigned by auth.js once the real Sheet data loads (only if
// the Sheet returns a non-empty list — see auth.js). Declaring them
// here as empty arrays (instead of in a separate data.js with sample
// values) guarantees they always exist as valid arrays even if that
// first Sheet fetch is briefly empty or fails, so those files never
// throw a ReferenceError. Previously used to also hold sample
// EMPLOYEES/CLIENTS/PROJECTS for DEMO_MODE — dropped since this app
// doesn't use DEMO_MODE.
let CLIENTS  = [];
let PROJECTS = [];
let SUBTASKS = []; // { pid, id, name } — read by form.js, reassigned by auth.js from master.subtasks
