...
async function apiGetMasterData() {
  // DEMO_MODE previously returned sample EMPLOYEES/CLIENTS/PROJECTS
  // here (from the now-removed data.js). This app doesn't use
  // DEMO_MODE, so that branch is gone — always fetch live.
...
// ── AUTH ──────────────────────────────────────────
// DEMO_MODE previously checked login against the sample EMPLOYEES
// array here (from the now-removed data.js). This app doesn't use
// DEMO_MODE, so that branch is gone — always authenticate live.
...
// ── CHANGE OWN PASSWORD (self-service, any logged-in employee) ────
async function apiChangeOwnPassword(uid, oldPw, newPw) {
  return sheetGET({
    action: 'changeOwnPassword',
    data:   encodeURIComponent(JSON.stringify({ uid, oldPw, newPw })),
  });
}
